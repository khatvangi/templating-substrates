...
class Mode6Population:
    ...
    def reproduce(self, parent_indices):
        new_surface = self.surface[parent_indices].copy()
        # Lossy inheritance: each position INDEPENDENTLY corrupted at high rate
        mu_surface = 0.10
        flips = self.rng.random(new_surface.shape) < mu_surface
        replacements = self.rng.integers(0, ALPHABET, size=new_surface.shape, dtype=np.int8)
        new_surface = np.where(flips, replacements, new_surface)
...
def run_experiment():
    ...
    progress_path = RESULTS_DIR / "test_d_progress.txt"
...
def evaluate_pass(history):
    ...
    c5 = plateaus.get("Mode6", 1) <= 0.45
    c6 = all(plateaus.get(lab, 1) <= 0.35 for lab in [
        "Mode1_bulk2", "Mode3_N2_bulk2", "Mode4_bulk2",
        "Mode5_N8_bulk2", "Mode6_bulk2"])
    c7 = (plateaus.get("Mode1", 0) > plateaus.get("Mode5_N8", 0) >
          max(plateaus.get("Mode6", 0), plateaus.get("Mode3_N2", 0)) >
          plateaus.get("Mode4", 0) - 0.05)
...
def main():
    ...
    save_csv(history, RESULTS_DIR / "test_d_results.csv")
    plot_fitness_trajectories(history, FIGURES_DIR / "test_d_fitness_trajectories.png")
    plot_ceiling_validation(history, FIGURES_DIR / "test_d_ceiling_validation.png")
    plot_distinct_phenotypes(history, FIGURES_DIR / "test_d_distinct_phenotypes.png")
    (RESULTS_DIR / "test_d_README.md").write_text(README_TEXT)
    ...
    crit_labels = [
        ...
        "C5: Mode6 plateau <= 0.45",
        "C6: all bulk controls <= 0.35",
        "C7: ordering Mode1 > Mode5 > Mode6/3 > Mode4",
    ]
